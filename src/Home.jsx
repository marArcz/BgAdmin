...
function Home() {
    ...
    const loadProducts = async () => {
        const { data, error } = await supabase.from("products").select("*,images(*),category(*)").order("id", { ascending: false })
    ...
    return (
        <div className="Home">
            ...
            <div className="container-fluid mt-3">
                <h3 className='mb-4 text-dark'>Products</h3>
                <div className="d-flex mb-3">
                    {showDelete && (<button type='button' onClick={deleteChecked} className="btn btn-sm btn-danger ">Delete Selected</button>)}
                    <button onClick={() => setIsAddFormOpen(true)} className='ms-auto btn btn-sm btn-success'><i className='bx bx-plus'></i> Add New</button>
                </div>
                            ...
                            {products ? (
                                products.map((product, index) => (
                                    <tr key={index}>
                                        ...
                                        <td><small className='text-dark fw-'>{index + 1}</small></td>
                                        <td><img src={product.images.length > 0 ? product.images[0].src : ""} width={50} alt="" className='img-thumbnail img-fluid me-3' /><small className='text-dark fw-'>{product.name}</small></td>
                                        <td><small className='text-dark fw- text-wrap'>{product.description}</small></td>
                                        <td><small className='text-dark fw-'>P {product.price}</small></td>
            ...
            {/* Add form modal */}
            <AddForm show={isAddFormOpen} handleClose={() => setIsAddFormOpen(false)} onSubmit={HandleAdd} data={{ categories }} />

            {/* edit form */}
            {productToEdit && (<EditForm show={isEditFormOpen} handleClose={() => setIsEditFormOpen(false)} onSubmit={HandleEdit} data={{ categories }} product={productToEdit} />)}
