...
const AddImage = async (productId, src) => {
    const added = await supabase.from("images").insert({ productId, src })
    return added;
}

const DeleteImage = async (id) => {
    const deleted = await supabase.from("images").delete().match({ id })
    return deleted;
}

const getImages = async (productId) => {
    const images = await supabase.from("images").select("*").match(productId)
    return images;
}
...
const DeleteProduct = async (id) => {
    const deletedImages = await supabase.from("images").delete().match({ productId: id })
